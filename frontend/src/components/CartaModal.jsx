...
export const CartaModal = ({ carta, open, handleClose, cantidad, coleccionId}) => {
  ...
  const sale = async () => {
    ...
    try {
      const { data, isLoading, error } = await fetchData('mercado/insert','POST', token, body);
      if(error === 400) {
        alert(data);
        return
      };  
      ...
      try{
        ...
        const { data, isLoading, error  } = await fetchData(`/coleccion/${coleccionId}`, 'PUT', token, body2);
        if(error === 400) {
          alert(data);
          return
        };  
  ...
  const info = () =>{
    ...
    <Box className="texto">
      ...
      <p className="linea"><b>CANTIDAD:</b> <span>{cantidad}</span></p>

    </Box>
    <Box className="botones">
      <button onClick={handleClose} className="cerrar">CERRAR</button>
      {cantidad > 1 && <button  onClick={handleVenta}  className="vender">VENDER</button>}
    </Box>
  </Box>)
  }
  ...
  const venta = ()=>{
    ...
    <Box className="botones">
      <button onClick={handleCancel} className="cerrar">CANCELAR</button>
     <button className="vender" onClick={sale}>VENDER</button>
    </Box>
  </Box>)
  }
